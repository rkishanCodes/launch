...
const palette = {
  color1: new THREE.Color(0x001a33), // Deep midnight blue
  color2: new THREE.Color(0x024b6e), // CAIAS primary blue
  line: new THREE.Color(0xffb72b), // CAIAS golden orange
  core: new THREE.Color(0xffd700), // Bright gold core
};
...
// CAIAS Logo Component - 3D Circular Emblem
function CAIASLogo({ pulse }) {
  const groupRef = useRef();
  const logoRef = useRef();
  const materialRef = useRef();
  const glowRingRef = useRef();

  // Create circular logo geometry with emblem design
  const logoGeometry = useMemo(() => {
    const shape = new THREE.Shape();
    const outerRadius = 30;

    // Create circular emblem shape
    shape.absarc(0, 0, outerRadius, 0, Math.PI * 2, false);

    // Inner circle (cutout for depth)
    const holePath = new THREE.Path();
    holePath.absarc(0, 0, outerRadius - 4, 0, Math.PI * 2, true);
  ...
  // Create torch/flame symbol in center
  const torchGeometry = useMemo(() => {
    const shape = new THREE.Shape();

    // Simplified torch flame shape
    shape.moveTo(0, 15);
    shape.quadraticCurveTo(3, 18, 0, 22);
    shape.quadraticCurveTo(-3, 18, 0, 15);
  ...
  // Create book/pages symbol
  const bookGeometry = useMemo(() => {
    const bookGroup = new THREE.Group();

    // Book base
    const bookBase = new THREE.BoxGeometry(20, 2, 8);
  ...
  const logoMaterial = useMemo(() => {
    return new THREE.ShaderMaterial({
      uniforms: {
        time: { value: 0 },
        pulse: { value: 0 },
        color1: { value: new THREE.Color(0x024b6e) }, // CAIAS blue
        color2: { value: new THREE.Color(0xffb72b) }, // CAIAS orange
        glowColor: { value: new THREE.Color(0xffd700) }, // Gold glow
      },
      vertexShader: `
        uniform float time;
        uniform float pulse;
        ...
        void main() {
          ...
          float displacement = snoise(position * 0.05 + time * 0.3) * 1.5;
          vNoise = snoise(position * 0.2 + time);
          vec3 newPosition = position + normal * (displacement + vNoise * (1.0 + pulse * 5.0));
          gl_Position = projectionMatrix * modelViewMatrix * vec4(newPosition, 1.0);
        }
      `,
      fragmentShader: `
        ...
        void main() {
          ...
          float intensity = pow(0.7 - abs(vNoise), 1.5);
          vec3 finalColor = mix(baseColor, glowColor, pulse * 0.5);
          gl_FragColor = vec4(finalColor, 0.95) * intensity + vec4(glowColor, 1.0) * pulse * fresnel;
        }
      `,
      transparent: true,
      blending: THREE.AdditiveBlending,
  ...
  useFrame((state) => {
    ...
    if (glowRingRef.current) {
      glowRingRef.current.scale.set(
        1 + pulse.strength * 0.3,
        1 + pulse.strength * 0.3,
        1
      );
      glowRingRef.current.material.opacity = 0.3 + pulse.strength * 0.4;
    }
  });
  ...
  return (
    <group ref={groupRef}>
      ...
      <mesh ref={logoRef} geometry={logoGeometry}>
        <primitive object={logoMaterial} ref={materialRef} attach="material" />
      </mesh>

      {/* Torch/Flame symbol */}
      <mesh geometry={torchGeometry} position={[0, 0, 4]}>
        <meshStandardMaterial
          color={0xffb72b}
          emissive={0xff9500}
          emissiveIntensity={0.5 + pulse.strength}
          metalness={0.8}
          roughness={0.2}
        />
      </mesh>
      ...
      {/* Book base */}
      <mesh geometry={bookGeometry} position={[0, -10, 3]}>
        <meshStandardMaterial
          color={0xffffff}
          emissive={0x024b6e}
          emissiveIntensity={0.2 + pulse.strength * 0.3}
          metalness={0.4}
          roughness={0.3}
        />
      </mesh>
      ...
      <mesh position={[0, 0, -1]}>
        ...
        <meshStandardMaterial
          color={0x024b6e}
          emissive={0x00f5ff}
          emissiveIntensity={0.5 + pulse.strength * 0.8}
          metalness={0.9}
          roughness={0.1}
        />
      </mesh>

      {/* Text ring effect (placeholder for "Light to Enlighten" motto) */}
      {[...Array(12)].map((_, i) => (
        <mesh
          key={i}
          position={[
            Math.cos((i / 12) * Math.PI * 2) * 28,
            Math.sin((i / 12) * Math.PI * 2) * 28,
            2,
          ]}
        ...
        >
          ...
          <meshStandardMaterial
            color={0xffd700}
            emissive={0xffb72b}
            emissiveIntensity={0.8 + pulse.strength}
            metalness={0.9}
            roughness={0.1}
          />
        </mesh>
      ...
      {/* Aura/Glow effect */}
      <mesh scale={[1.4, 1.4, 1.4]} position={[0, 0, -2]}>
...
export default function StellarFlare() {
  ...
  return (
    <>
      ...
      <pointLight position={[0, 0, 0]} intensity={1.5} color={0xffb72b} />
      <Nebula />
      <group ref={starGroupRef}>
        <CAIASLogo pulse={pulseRef.current} />
        <FlareLines pulse={pulseRef.current} starGroupRef={starGroupRef} />
      </group>
      <StellarDust starGroupRef={starGroupRef} />
      <PostProcessing />
